...
const { sendMoney, requestMoney, viewRequests, approveRequest, viewRequestsByTXID } = require('./modules/transaction');
    ...
    rl.on('line', async (input) => {
        ...
        if (command === 'help') {
            ...
            socket.write(
                ...
                `createuser <username> <password> [role] - Create a new user (Admin and Teller only)\n` +
        ...
        else if (command === 'createuser' && ((await validateRole()) === 'Admin' || (await validateRole()) === 'Teller'))
        {
            // Only allow admins to create new users
            const [newUsername, newPassword, newRole = 'User'] = args; // Default role to 'User'
            if (!newUsername || !newPassword) {
                socket.write(`Usage: createuser <username> <password> [role]\nAlphaBank(${username}:${role})> `);
                return;
            }
            ...
            await updateUserRole(targetUsername, newRole);
            ...
            const response = await requestMoney(username, toUsername, parseFloat(amount));
            ...
            if (!Array.isArray(pendingTransactions) || pendingTransactions.length === 0) {
                ...
                pendingTransactions.forEach(tx => {
                    socket.write(`Transaction ID: ${tx.transactionId}, Amount: $${tx.amount}, Status: ${tx.type}\n`);
                });
            }
